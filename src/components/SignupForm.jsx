...
const SignupForm = () => {
  ...
  const TIMEZONE_OPTIONS = [
    'Eastern Time','Central Time','Mountain Time','Arizona Time',
  ...
  const handleChange = (e) => {
    ...
    if (name === 'messages_per_day') {
      ...
      setShowCustom2(two);
      if (!two) {
        setFormData(prev => ({ ...prev, time_2: '', custom_msg_2: '' }));
      }
    }
  };
  ...
  const parseTime = (timeString) => {
    ...
    return match ? { hour: match[1], minute: match[2], period: match[3] }
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          tz: 'Eastern Time', messages_per_day: '1',
          time_1: '', time_2: '', custom_msg_1: '', custom_msg_2: '',
          consent: false,
        });
        setShowTime2(false);
  ...
  return (
    <form onSubmit={handleSubmit} className="signup-form" aria-label="Signup Form">
      ...
      <div className="form-group">
        ...
        <input type="text" id="nickname" name="nickname" value={formData.nickname} onChange={handleChange} placeholder="We'll use this if provided" />
      </div>

      <div className="form-group">
        <label htmlFor="email">Email</label>
        <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} placeholder="Optional - for account recovery only" />
      </div>

      <div className="form-group">
        <label htmlFor="phone">Phone Number *</label>
        <input type="tel" id="phone" name="phone" value={formData.phone} onChange={handleChange} required placeholder="e.g., 2125551234" />
